...
const
  ...
  ssscUnknownClassS =
    'Container class name %s read from stream is unregistered';
  ...
  ssscWrongNodeClassS =
    'Node class name on stream differs from object''s node class';
  ssscBadCompareS   = 'Unable to assign this compare function now';
  ssscTooManyColsS  =
    'Cannot assign a matrix with more than 1 column to an array';
  ssscBadColCountS  =
    'Can only assign a matrix to a virtual matrix if column counts are equal';
  ssscBadElSizeS    =
    'Can only assign a matrix to a virtual matrix if element sizes are equal';
  ssscBadDupsS      =
    'Can only set Duplicates to False in an empty sorted collection';
  ...
  ssscShellVersionErrorS =
    'Operation not supported in this version of the shell';
  ...
  ssscTrayIconCloseS    = '&Закрыть';
  ssscTrayIconRestoreS  = '&Восстановить';
  ...
  ssscTrayIconDuplicateS =
    'Cannot have more than one StTrayIcon per application';
  ...
  ssscNilStreamS      =
    'Buffered/text stream: Attempted to read, write, or seek and underlying stream is nil';
  ssscNoSeekForReadS  =
    'Buffered/text stream: Could not seek to the correct position in the underlying stream (for read request)';
  ssscNoSeekForWriteS =
    'Buffered/text stream: Could not seek to the correct position in the underlying stream (for write request)';
  ssscCannotWriteS    =
    'Buffered/text stream: Could not write the entire buffer to the underlying stream';
  ssscBadTerminatorS  =
    'Text stream: Case statement was used with a bad value of LineTerminator';
  ssscBadLineLengthS  =
    'Text stream: Length of a fixed line must be between 1 and 4096 bytes';
  ssscCannotSetSizeS  =
    'Buffered/text stream: Cannot set the size of the underlying stream (needs OnSetStreamSize event)';

  ssscUnknownErrorS    = 'Unknown error creating a pattern token';
  ssscExpandingClassS  = 'Problem in expanding character class';
  ssscAlternationFollowsClosureS =
    'Alternation cannot immediately follow a closure marker';
  ssscUnbalancedParensS = 'Unbalanced nesting parentheses';
  ssscFollowingClosureS =
    'Closure cannot immediately follow BegOfLine, EndOfLine or another closure';
